...
def dpll(formula):
    # formulo v CNF obliki pretvorimo v seznam slovarjev
...
def ciste_pojavitve(list_formula,znane_spr={}):
        ...
        for i in list_formula:
            for k,l in i.items():
                if k not in pojavitve:
                    ...
                    if pojavitve[k]!=l:
                        del pojavitve[k]
        znane_spr.update(pojavitve)                
        for j in list_formula:
            for m in j.keys():
                if m in pojavitve:
                    list_formula.remove(j)
                    s=True
                    break
